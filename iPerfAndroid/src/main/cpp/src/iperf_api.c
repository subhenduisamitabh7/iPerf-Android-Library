...
{LOGI("start parse arguments---argc=%d---%s----\n",argc,argv[0]);
        int d=0;
    for(d=0;d<argc;d++){
        LOGI("start parse arguments argv[%d]=%s\n",d,argv[d]);
...
iperf_new_stream(struct iperf_test *test, int s, int sender)
{
    ...
    if (test->tmp_template) {
		...
		#endif
		//snprintf(template, sizeof(template) / sizeof(char), "%s/iperf3.XXXXXX", "/data/local/tmp");
        snprintf(template, sizeof(template) / sizeof(char), "%s/iperf3.XXXXXX", "/data/data/com.example.iperf");
    }
